...
def test_pip_configure_without_ssl(tmp_path: Path):
    # Arrange
    venv_dir = tmp_path / ".venv"
    venv_dir.mkdir(parents=True)
    my_venv = instantiate_os_specific_venv(venv_dir)
...
def test_get_inputs_includes_relevant_files(tmp_path: Path, expected_file: str):
    # Arrange
...
def test_python_version_marker_written_after_package_manager_run(tmp_path: Path, bootstrap_config, bootstrap_env):
    # Arrange
    venv_dir = tmp_path / ".venv"
...
def test_venv_deleted_when_no_marker_found(tmp_path: Path, bootstrap_config, bootstrap_env):
    # Arrange
    venv_dir = tmp_path / ".venv"
    venv_dir.mkdir(parents=True)
...
def test_venv_deleted_when_python_version_changes(tmp_path: Path, bootstrap_config, bootstrap_env):
    # Arrange
    venv_dir = tmp_path / ".venv"
    venv_dir.mkdir(parents=True)
    marker_file = venv_dir / VENV_PYTHON_VERSION_MARKER
...
def test_venv_preserved_when_python_version_matches(tmp_path: Path, bootstrap_config, bootstrap_env):
    # Arrange
    venv_dir = tmp_path / ".venv"
    venv_dir.mkdir(parents=True)
    marker_file = venv_dir / VENV_PYTHON_VERSION_MARKER
