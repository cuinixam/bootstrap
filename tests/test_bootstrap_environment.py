...
def test_create_bootstrap_environment_get_name(bootstrap_env: CreateBootstrapEnvironment):
    # Act
    name = bootstrap_env.get_name()

    # Assert
    assert name == f"create-bootstrap-environment-{bootstrap_env.env_hash}"


def test_create_bootstrap_environment_get_inputs_empty(bootstrap_env: CreateBootstrapEnvironment):
    # Act
    inputs = bootstrap_env.get_inputs()

    # Assert - bootstrap env has no file-based inputs
    assert inputs == []
...
def test_create_virtual_environment_get_install_command(tmp_path: Path, project_dir: Path, config_kwargs, expected_command):
    # Arrange
