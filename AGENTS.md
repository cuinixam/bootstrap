...
Follow the project's `.ruff` settings in `pyproject.toml`:

- Enabled: `flake8-bugbear`, `flake8-comprehensions`, `flake8-bandit`, `pycodestyle`, `pyupgrade`, `isort`
- Ignored: Missing docstrings in tests, imperative mood requirements
- Security: Use `# nosec` only when subprocess usage is validated
...
- **Bucket URLs**: Use raw GitHub URLs for bucket manifests
...
1. Add field to `BootstrapConfig` dataclass
2. Update `from_json_file()` to parse new field
3. Add test in `test_bootstrap_environment.py`
4. Document in README.md

### Adding a New Executor

1. Subclass `Executor` ABC
2. Implement `execute()` method
3. Add tests in `test_executor.py`
4. Register in executor factory (if applicable)

### Modifying Bootstrap Logic

1. Update `bootstrap.py` main orchestration
2. Ensure idempotency (check markers like `.bootstrap-complete`)
3. Add integration test in `integration.Tests.ps1`
...
- ❌ Shell injection vulnerabilities in subprocess calls
- ❌ Ignore security warnings (address or justify with `# nosec`)

**PowerShell:**

- ❌ Use `Invoke-Expression` with untrusted input (command injection risk)
- ❌ Ignore execution policy issues (document requirements)
- ❌ Hardcode paths (use `Join-Path` and relative paths)
- ❌ Use positional parameters (always use named parameters for clarity)
- ❌ Modify global environment permanently without user consent

**General:**

- ❌ Add unnecessary dependencies (keep bootstrap lightweight)
- ❌ Break Windows compatibility
- ❌ Skip testing (both Python pytest and PowerShell Pester)

### Adding Scoop Configuration

1. Add configuration to default config in `Get-BootstrapConfig`
2. Handle merge logic for custom `bootstrap.json` values
3. Update Scoop installation in `Install-Scoop` function
...
- [ ] Important steps logged (`logger.info()`)

**PowerShell:**

- [ ] Functions use approved Verb-Noun naming
- [ ] Comment-based help added for public functions
- [ ] No unvalidated `Invoke-Expression` usage
- [ ] Pester tests pass (`.\tests\bin\test.ps1`)
- [ ] External commands properly mocked in tests
- [ ] `-ErrorAction` used explicitly where appropriate
