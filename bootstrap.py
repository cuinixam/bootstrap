...
class WindowsVirtualEnvironment(VirtualEnvironment):
    ...
    def python_path(self) -> Path:
        return self.venv_dir.joinpath("Scripts/python.exe")
...
class UnixVirtualEnvironment(VirtualEnvironment):
    ...
    def python_path(self) -> Path:
        return self.venv_dir.joinpath("bin/python")
...
class CreateBootstrapEnvironment(Runnable):
    ...
    def get_name(self) -> str:
        return f"create-bootstrap-environment-{self.env_hash}"
...
class CreateVirtualEnvironment(Runnable):
    ...
    def __init__(
        self,
        root_dir: Path,
        bootstrap_env: Optional[CreateBootstrapEnvironment] = None,
        ...
        self.config = bootstrap_env.config if bootstrap_env else (config or BootstrapConfig())
    ...
    def run(self) -> int:
        # Check Python version compatibility before any operations
        self._check_python_version_compatibility()

        # Ensure package manager will create venv in-project
        ...
        if self.bootstrap_env:
            ...
            self.bootstrap_env.virtual_env.run(install_command, capture_output=True, cwd=self.root_dir)
            ...
            # Fallback to old behavior: create venv manually and install package manager
            logger.info("No bootstrap environment provided, using legacy installation method.")

            # Create venv manually in legacy mode
            if not self.virtual_env.pip_path().exists():
            ...
            install_command = self._get_install_command()
            self.virtual_env.run(install_command)

            # Write version marker after legacy setup
            current_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
            self._write_python_version_marker(current_version)
    ...
    def get_inputs(self) -> List[Path]:
        ...
        # Include the bootstrap environment marker as input
        if self.bootstrap_env:
            venv_relevant_files.append(str(self.bootstrap_env.marker_file))
